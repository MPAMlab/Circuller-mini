...
// Define the LED order
int ledOrder[8] = {3, 2, 1, 0, 7, 6, 5, 4};
//int ledOrder[8] = {0, 1, 2, 3, 4, 5, 6, 7};
enum {
  LedGs8Bit = 0x31,//49
...
void setup() {
  ...
  leds[ledOrder[req.index]] = 0xFFFFFF;
  FastLED.delay(1000);
...
void loop() {
  ...
  switch (packet_read()) {
    ...
    case LedGs8BitMulti:
      ...
      for (uint8_t i = req.start; i < req.end; i++) {
          leds[ledOrder[i]] = CRGB(req.mr, req.mg, req.mb);
      }
  ...
  for (uint8_t i = led_start; i <= led_end; i++) {
      leds[ledOrder[i]] = blend(fade_prev, fade_taget, progress);
  }
  FastLED.show();
